...
{
	extern TX_EVENT_FLAGS_GROUP mdns_event_group;
  /* Check cache state. */
  if (state == NX_MDNS_CACHE_STATE_FULL)
  {
    MSG_DEBUG("Cache Full!!!\n");
    /* Set event flags 0 */
    tx_event_flags_set(&mdns_event_group, 0x1, TX_OR);
  }
  else
  {
    MSG_DEBUG("Cache Full With Fragment!!!\n");
    tx_event_flags_set(&mdns_event_group, 0x10, TX_OR);
  }
  ...
  if (cache_type == NX_MDNS_CACHE_TYPE_LOCAL)
  {
    ...
    /* Set event flags 0 */
    tx_event_flags_set(&mdns_event_group, 0x100, TX_OR);
  }
  else
  {
...
//    nx_mdns_peer_cache_clear(mdns_ptr);
    tx_event_flags_set(&mdns_event_group, 0x1000, TX_OR);
